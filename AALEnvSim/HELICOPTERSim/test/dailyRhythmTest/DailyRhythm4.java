...
public class DailyRhythm4 {
	...
	static class SleepStateProbabilityFunction extends StateEdgeProbabilityFunctionSpecification.Function {
		...
		@Override
		public TreeMap<Edge, Double> evaluate(Type type,State state, Set<Edge> edgeSet, Set<Edge> applicableEdgeSet) {
			Instant lastSleepStateChangeTime=(Instant) state.getStateMachine().getStateMachineGroup().getVariable("lastSleepStateChangeTime").get(0);
			TreeMap<Edge,Double> result=new TreeMap<Edge,Double>();
			for (Edge edge:edgeSet) {
				result.put(edge, 0.0);
			}
	...
	public static class TestRunnable implements Runnable {
		//static Mode.Type type;
		 Set<Mode.Type> typeSet=new HashSet<Mode.Type>();
		 Set<ModeState> modeStateSet=new HashSet<ModeState>();
		 Configuration cfg;
		 double awakeIndex=0.1;
		 String virtualSubject;
		...
		@Override
		public void run() {
			final String[] virtualSubjects={this.virtualSubject};
			...
			cfg=new Configuration(typeSet,Instant.parse("2015-01-01T00:00:00Z"),Instant.parse("2015-02-28T00:00:00Z")) {
				...
				public void initialize(StateMachineSystem sms,EventType tick) {
					...
					@SuppressWarnings("unchecked")
					Variable<Instant> lastSleepStateChangeTime=(Variable<Instant>) theSmg.getOrCreateVariable(Variable.Type.Object,"lastSleeoStateChangeTime");
					lastSleepStateChangeTime.add(Instant.parse("2014-12-31T08:00:00Z"));
...
//					sleepInterval.put(sleepMode[4],remaining*0.1);
					{
						for ( int i=0; i<sleepMode.length; ++i) {
							...
							Action goToNextAction=new Action("goTo_"+endNameOfGoto+"_Action",sms) {
								...
								public boolean evaluate(StateMachine sm) {
									final Fatigue fatigue=((Fatigue)sm.getStateMachineGroup().getVariable("fatigue").elementAt(0));
									((Variable<Instant>)sm.getStateMachineGroup().getVariable("lastSleepStateChangeTime")).setElementAt((Instant)sm.getStateMachineSystem().getEngineData().getTime().getTime(),0);
	...
	public static class VirtualSubjectSpecification {
		String name;
		Vector<Long> seedVector;
	...
	public void test() {
		try {
			...
			VirtualSubjectSpecification vss[]={
					new VirtualSubjectSpecification()
			};
			Vector<Thread> threadVector=new Vector<Thread>();
			for (VirtualSubjectSpecification vs: vss) {
				TestRunnable tr=new TestRunnable(vs.getName());
